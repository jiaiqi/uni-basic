<template>
	<view class="coupon-list-wrap">
		...
		<view v-if="couponValidList.length > 0" class="coupon-list-main">
			<view class="row">
				<view v-for="(item,index) in couponValidList" class="carrier">
					<view class="left">
						<view class="title">
							{{item.coupon_name}}
						</view>
						<view class="term">
							<view style="margin-bottom: 2px;" class="term_time">
								使用时间：{{item.used_start_time}}
							</view>
							<view class="term_time">
								过期时间：{{item.used_end_time}}
							</view>
						</view>
					...
					<view class="right" :class="isLose?'is-lose':isUse?'is-use':''">
						...
						<view class="criteria">
							满{{item.consume_amount}}使用
						</view>
						<view @click="toUse(item)" v-if="TabCur == 1" class="use">
							去使用
						</view>
						<view v-if="TabCur == 2" class="use">
							已使用
						</view>
						<view @click="drawCoupon(item)" v-if="TabCur == 0" class="use">
							领取
						</view>
					</view>
				</view>
